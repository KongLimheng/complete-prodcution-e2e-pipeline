pipeline{
    ...
    stages{
        ...
        stage("Checkout from SCM"){
            steps {
                git branch: 'main', credentialsId: 'github', url: 'https://github.com/KongLimheng/complete-prodcution-e2e-pipeline'
            }
        ...
        stage("Sonarqube Analysis") {
            steps {
                script {
                    withSonarQubeEnv(credentialsId: 'jenkins-sonarqub') {
                        sh "mvn sonar:sonar"
                    }
                }
            }
        ...
        stage("Quality Gate") {
            steps {
                script {
                    waitForQualityGate abortPipeline: false, credentialsId: 'jenkins-sonarqub'
                }
            }
        }
